        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tab_title != null) {
               ...
               if (i == mPager.getCurrentItem()) {
                	tab_title.setTextColor(getResources().getColor(R.color.tabs_text));
                    Utility.CURRENT_SUB_CAT_TITLE= tab_title.getText().toString();
    				SpannableString content = new SpannableString(Utility.CURRENT_SUB_CAT_TITLE);
    				content.setSpan(new UnderlineSpan(), 0,  Utility.CURRENT_SUB_CAT_TITLE.length(), 0);
    				tab_title.setText(content);
                } else {
                	tab_title.setTextColor(getResources().getColor(R.color.tabs_text_selected));
                }
                // end of the change
