...
import in.playcer.R;
import in.playcer.ui.activities.CitrusUIActivity;
...
public class CitrusFlowManager {
    ...
    public static final String KEY_FROM_EVENTS_OR_SLOTS = "key_events_or_slots";
     ...
     */
    public static void startShoppingFlowStyle(Context context, String email, String phone, String amount, int style, Activity _activity, String _From_events_Slots) {
        if (TextUtils.isEmpty(email)) {
            Utility.showCustomToast(context.getString(R.string.err_email_empty), _activity);
            //Toast.makeText(context, context.getString(R.string.err_email_empty), Toast.LENGTH_SHORT).show();
        }
        else if(!Utils.isValidEmail(email)){
            Utility.showCustomToast("Email address is not valid", _activity);
            //Toast.makeText(context, "Email address is not valid", Toast.LENGTH_SHORT).show();
        }
        else if (TextUtils.isEmpty(phone) || phone.length() != 10) {
            Utility.showCustomToast(context.getString(R.string.err_phone_empty), _activity);
            //Toast.makeText(context, context.getString(R.string.err_phone_empty), Toast.LENGTH_SHORT).show();
        } else if (TextUtils.isEmpty(amount) || Double.parseDouble(amount) == 0) {
            Utility.showCustomToast(context.getString(R.string.err_amount_empty), _activity);
            //Toast.makeText(context, context.getString(R.string.err_amount_empty), Toast.LENGTH_SHORT).show();
            ...
            intent.putExtra(KEY_FROM_EVENTS_OR_SLOTS, _From_events_Slots);
     ...
     * @param amount  amount to be paid.
     */
    public static void startShoppingFlow(Context context, String email, String phone, String amount) {
        if (TextUtils.isEmpty(email)) {
